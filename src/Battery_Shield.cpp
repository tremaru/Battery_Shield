...
bool	Battery_Shield::_begin(float res, float cua){																	//	Параметр: значение сопротивления в Ом, [значение КПД в %].
...
bool	Battery_Shield::off(void){																						//	Параметр: отсутствует.
			if(			selI2C->readBytes(valAddr, IP5108_SYS_CTL4,  valRegistor, 1) ){									//	Если удалось прочитать значение регистра IP5108_SYS_CTL4 в элемент массива valRegistor[0], то ...
				return	selI2C->writeByte(valAddr, IP5108_SYS_CTL4, (valRegistor[0]|bit(7)) );							//	Сохраняем значение в регистр IP5108_SYS_CTL4 из элемента массива valRegistor[0], предварительно установив 7 бит.
...
void	Battery_Shield::charging(bool flgEnCharging){																	//	Параметр: флаг разрешения работы зарядного устройства - да/нет.
			...
			if(	selI2C->readBytes(valAddr, IP5108_SYS_CTL0, valRegistor, 1) ){											//	Если удалось прочитать значение регистра IP5108_SYS_CTL0 в элемент массива valRegistor[0], то ...
				if(flgEnCharging)	{selI2C->writeByte(valAddr, IP5108_SYS_CTL0, (valRegistor[0]|  bit(1) ) );}			//	Сохраняем значение в регистр IP5108_SYS_CTL0 из элемента массива valRegistor[0], предварительно установив 1 бит.
				else				{selI2C->writeByte(valAddr, IP5108_SYS_CTL0, (valRegistor[0]&(~bit(1))) );}			//	Сохраняем значение в регистр IP5108_SYS_CTL0 из элемента массива valRegistor[0], предварительно сбросив   1 бит.
			}																											//
}																														//
																														//
...
uint8_t	Battery_Shield::getState(void){																					//	Параметр: отсутствует.
			_noSleep();																									//	Запрещаем модулю отключаться при отсутствии нагрузки.
			if(	selI2C->readBytes(valAddr, IP5108_REG_READ1, valRegistor, 1) ){											//	Если удалось прочитать значение регистра IP5108_REG_READ1 в элемент массива valRegistor[0], то ...
...
float	Battery_Shield::_ADC(uint8_t reg){																				//	Параметр: номер первого регистра.
			_noSleep();																									//	Запрещаем модулю отключаться при отсутствии нагрузки.
			if(	selI2C->readBytes(valAddr, reg, valRegistor, 2) ){														//	Если удалось прочитать 2 байта начиная с регистра reg в массив valRegistor, то ...
...
//			Ждём ответа модуля:																							//
			if(selI2C->readBytes(valAddr,IP5108_SYS_CTL1,valRegistor,1)){												//	Если удалось прочитать значение регистра IP5108_SYS_CTL1 в элемент массива valRegistor[0], то ...
			if(valRegistor[0] & bit(1)){																				//	Если 1 бит байта ответа модуля установлен в «1», то ...
				selI2C->writeByte(valAddr, IP5108_SYS_CTL1, (valRegistor[0]&(~bit(1))) );								//	Сохраняем значение valRegistor[0] в регистр IP5108_SYS_CTL1, предварительно сбросив 1 бит.
